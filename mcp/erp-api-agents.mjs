...
export class ErpApiAgents {
  ...
  async run({ projectId, role, task = "", queryId = "" }) {
    ...
    try {
      ...
      this.store.recordModelCallTelemetry({ projectId, queryId: effectiveQueryId, provider: policy.provider, model, reasoningEffort: parameters.reasoning_effort, skillVersion: startedRun.provenance.skill_version, applicationVersion: "4.3.0", inputTokens: usage.inputTokens, cachedInputTokens: usage.cachedInputTokens, outputTokens: usage.outputTokens, reasoningTokens: usage.reasoningTokens, durationMs, attempt: 1, result: "completed", usageSource: "provider_response" });
      ...
      const durationMs = Date.now() - startedAt;
      try { this.store.recordModelCallTelemetry({ projectId, queryId: effectiveQueryId, provider: policy.provider, model, reasoningEffort: parameters.reasoning_effort, skillVersion: startedRun.provenance.skill_version, applicationVersion: "4.3.0", inputTokens: null, cachedInputTokens: null, outputTokens: null, reasoningTokens: null, durationMs, attempt: 1, result: error?.name === "AbortError" ? "timeout" : "failed", error: error?.message || String(error), usageSource: "provider_response" }); } catch { /* Preserve original error. */ }
