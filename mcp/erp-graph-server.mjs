...
import { ErpODataClient } from "./erp-odata-client.mjs";

const store = new ErpGraphStore();
const apiAgents = new ErpApiAgents(store);
const odata = new ErpODataClient();

const server = new McpServer(
  { name: "newagent-1c", version: "4.3.0" },
  {
    instructions:
...
server.registerTool(
  "get_odata_status",
  {
    title: "Get 1C OData status",
    description:
      "Show safe 1C OData connection settings without returning credentials. Optionally read remote $metadata.",
    inputSchema: {
      remote: z.boolean().default(false),
    },
    annotations: odataReadOnly,
  },
  async ({ remote }) => result(await odata.status({ remote })),
);

server.registerTool(
  "list_odata_entities",
  {
    title: "List 1C OData entities",
    description: "Read $metadata from the configured standard 1C OData interface and list published EntitySets.",
    inputSchema: {},
    annotations: odataReadOnly,
  },
  async () => result(await odata.listEntities()),
);

server.registerTool(
  "query_odata",
  {
    title: "Query 1C OData",
    description:
      "Read a bounded collection from the standard 1C OData interface. This tool never creates, updates or deletes 1C data.",
    inputSchema: {
      entity: z.string().min(1).max(200),
      select: z.array(z.string().min(1).max(200)).max(100).default([]),
      filter: z.string().max(4_000).default(""),
      order_by: z.string().max(4_000).default(""),
      expand: z.string().max(4_000).default(""),
      top: z.number().int().min(1).max(1_000).default(100),
      skip: z.number().int().min(0).max(1_000_000).default(0),
    },
    annotations: odataReadOnly,
  },
  async (input) => result(await odata.query(input)),
);
