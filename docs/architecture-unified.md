...
OData реализован двумя тонкими адаптерами общего read-only контракта для Python CLI
и MCP. Учётные данные остаются в окружении, в диагностике возвращаются только
признаки их наличия. MCP не получает операций записи: он является дополнительным
интерфейсом к графу, проектам и данным опубликованной базы 1С.
