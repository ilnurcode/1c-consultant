...
def parser() -> argparse.ArgumentParser:
    ...
    odata = commands.add_parser("odata", help="Чтение данных 1С через стандартный OData")
    odata_commands = odata.add_subparsers(dest="odata_command", required=True)
    odata_status = odata_commands.add_parser("status", help="Проверить настройку OData")
    odata_status.add_argument(
        "--remote", action="store_true", help="Выполнить безопасный запрос $metadata к 1С"
    )
    odata_commands.add_parser("entities", help="Показать опубликованные OData-сущности")
    odata_query = odata_commands.add_parser("query", help="Прочитать коллекцию OData")
    odata_query.add_argument("entity", help="Имя EntitySet из команды entities")
    odata_query.add_argument(
        "--select", action="append", default=[], help="Поле результата; можно повторить"
    )
    odata_query.add_argument("--filter", dest="filter_expression", default="")
    odata_query.add_argument("--orderby", dest="order_by", default="")
    odata_query.add_argument("--expand", default="")
    odata_query.add_argument("--top", type=int, default=100)
    odata_query.add_argument("--skip", type=int, default=0)
...
def dispatch(app, args) -> Any:
    ...
    if command == "runtime-status":
        return {
            ...
            "odata": app.odata.status(),
            "request_path": "python",
            "interfaces": ["opencode", "codex"],
        }
    if command == "odata":
        if args.odata_command == "status":
            return app.odata.status(remote=args.remote)
        if args.odata_command == "entities":
            return app.odata.list_entities()
        if args.odata_command == "query":
            return app.odata.query(
                args.entity,
                select=args.select,
        ...
        raise ValueError("Неизвестная команда odata")
