...
from consultant_cli.services.connections import ConnectionProfile, normalize_web_url
from consultant_cli.services.automation import CustomerOrderAutomation, CustomerOrderRequest
...
def run_menu(app: Application) -> int:
    while True:
        ...
        ui.menu(
            [
                ("1", "Автоматизация 1С", "Выполнить сценарий в 1С и проверить результат"),
                ("2", "Подключения к базам 1С", "Добавить, выбрать или проверить базу"),
                ("3", "Проекты", "Создать проект или продолжить существующий"),
                ("4", "Настройки и справка", "Помощь, система и расширенные настройки"),
...
def projects_menu(app: Application) -> None:
    while True:
        ui.clear()
        ui.header("Проекты", breadcrumb="Главное меню › Проекты")
        ui.menu(
            [
                ("1", "Создать проект", "Мастер инструкции и схемы"),
                ("2", "Мои проекты", "Открыть или продолжить проект"),
...
def technical_odata_menu(app: Application) -> None:
    """Read-only diagnostics, intentionally hidden from everyday workflow."""
    while True:
        ui.clear()
        ui.header("Технические данные OData", breadcrumb="Настройки › Расширенные › OData")
        ui.menu(
            [
                ("1", "Список сущностей", "Показать опубликованные EntitySet"),
                ("2", "Прочитать данные", "Запросить коллекцию с ограничением строк"),
                ("0", "Назад", "Вернуться в расширенные настройки"),
            ]
        )
        choice = ask("Выберите действие")
        if choice == "0":
        ...
        try:
            if choice == "1":
                result = app.odata.list_entities()
                ui.table(
                    ["Сущность", "Тип"],
                    [[item["name"], item["entity_type"]] for item in result["entities"]],
                )
            elif choice == "2":
                entity = ask_required("Имя сущности")
                result = app.odata.query(
                    entity,
                    filter_expression=ask("Фильтр OData (необязательно)"),
                    top=int(ask("Количество строк", "20")),
                )
                ui.panel(
                    f"{entity}: {result['count']} строк",
                    [json.dumps(row, ensure_ascii=False, default=str) for row in result["rows"]],
                    "green",
                )
            else:
                continue
        except (ConsultantError, ValueError) as exc:
            ui.error(str(exc))
        ui.pause()


def connections_menu(app: Application) -> None:
    while True:
        ui.clear()
        ui.header("Подключения к базам 1С", breadcrumb="Главное меню › Подключения")
        status = app.odata.status()
        active = app.active_connection.name if app.active_connection else "не выбрана"
        ui.panel(
            "Подключение",
            [
                f"Выбранная база: {active}",
                f"Сеанс: {'подключён' if status['configured'] else 'не начат'}",
                "Пароль не сохраняется и не передаётся AI.",
                "Адрес проверки данных определяется автоматически.",
            ],
            "cyan",
        )
        ui.menu(
            [
                ("1", "Добавить базу", "Указать название и адрес веб-клиента 1С"),
                ("2", "Выбрать базу", "Использовать её веб-адрес для проверки OData только на чтение"),
                ("3", "Проверить подключение", "Проверить доступ к данным без изменения 1С"),
                ("4", "Список баз", "Показать сохранённые адреса без паролей"),
                ("5", "Изменить базу", "Изменить название или адрес веб-клиента"),
                ("6", "Удалить базу", "Удалить только сохранённый адрес базы"),
                ("0", "Назад", "Вернуться в главное меню"),
            ]
        )
        choice = ask("Выберите действие")
        ...
        try:
            if choice == "1":
                name = ask_required("Название базы")
                address = ask_required("Адрес веб-клиента 1С")
                profiles = app.connections.list()
                profiles.append(ConnectionProfile(name=name, web_url=normalize_web_url(address)))
                app.connections.save(profiles)
                ui.success("База добавлена. Адрес проверки данных будет определён автоматически.")
            elif choice == "2":
                profile = choose_connection(app)
                if not profile:
                    continue
                app.odata = ODataService().for_service_url(profile.odata_url)
                app.active_connection = profile
                ui.success("База выбрана. OData подключён только для чтения; в веб-клиент войдите самостоятельно.")
            elif choice == "3":
                with ui.spinner("Проверяю OData 1С"):
                    remote = app.odata.status(remote=True)
                ui.success(f"Подключение работает. Сущностей: {remote['entity_count']}.")
            elif choice == "4":
                profiles = app.connections.list()
                ui.table(["Название", "Адрес веб-клиента"], [[p.name, p.web_url] for p in profiles])
            elif choice == "5":
                profile = choose_connection(app)
                if profile:
                    name = ask("Название базы", profile.name)
                    address = ask("Адрес веб-клиента 1С", profile.web_url)
                    updated = ConnectionProfile(name=name, web_url=normalize_web_url(address))
                    app.connections.save([updated if p == profile else p for p in app.connections.list()])
                    if app.active_connection == profile:
                        app.active_connection = updated
                        app.odata = ODataService()
                        ui.warning("Адрес изменён. Выберите базу заново для проверки данных.")
                    else:
                        ui.success("Сохранённый адрес базы изменён.")
            elif choice == "6":
                profile = choose_connection(app)
                if profile and yes_no(f"Удалить сохранённый адрес «{profile.name}»?"):
                    app.connections.save([p for p in app.connections.list() if p != profile])
                    if app.active_connection == profile:
                        app.active_connection = None
                        app.odata = ODataService()
                    ui.success("Сохранённый адрес базы удалён.")
            else:
                continue
        except (ConsultantError, ValueError) as exc:
            ui.error(str(exc))
...
def choose_connection(app: Application) -> ConnectionProfile | None:
    profiles = app.connections.list()
    if not profiles:
        ui.warning("Сначала добавьте базу 1С.")
        return None
    ui.table(["№", "Название", "Адрес"], [[str(i), p.name, p.web_url] for i, p in enumerate(profiles, 1)])
    raw = ask_required("Номер базы")
    try:
        return profiles[int(raw) - 1]
    except (ValueError, IndexError):
        ui.error("Выберите номер из списка.")
        return None


def automation_menu(app: Application) -> None:
    """Guide a consultant through the safe 1C automation workflow.

    Playwright MCP is hosted by Codex and controls only the web client. OData is
    deliberately read-only in this application and independently verifies the
    saved data. Keeping these responsibilities visible prevents accidental
    posting from the console menu.
    """
    ...
    while True:
        ...
        ui.header(
            "Автоматизация 1С",
            "Гибридный режим: действия в веб-клиенте + независимая проверка данных",
            "Главное меню › Автоматизация 1С",
        )
        configured = app.odata.status()
        playwright = playwright_mcp_status()
        ui.panel(
            "Как работает сценарий",
            [
                "1. Консультант входит в выбранную базу, не раскрывая пароль AI.",
                "2. Playwright MCP выполняет действия в веб-интерфейсе 1С.",
                "3. OData только читает базу и подтверждает результат.",
                "4. Проведение документа всегда требует отдельного подтверждения.",
            ],
            "cyan",
        )
        ui.panel(
            "Готовность",
            [
                f"База: {app.active_connection.name if app.active_connection else 'не выбрана'}",
                f"Вход в браузере: {'выполнен' if app.browser_login_completed else 'нужен'}",
                f"Проверка данных: {'готова' if configured['configured'] else 'войдите в базу'}",
                f"Playwright MCP: {playwright['message']}",
            ],
            "green" if configured["configured"] and playwright["configured"] else "yellow",
        )
        ui.menu(
            [
                ("1", "Войти в 1С", "Открыть защищённое окно входа через Playwright MCP"),
                ("2", "Выполнить заказ клиента", "Создать и записать непроведённый тестовый заказ"),
                ("3", "История запусков", "Показать последние сценарии без паролей"),
                ("4", "Провести документ", "Требует явного подтверждения и повторной проверки OData"),
                ("0", "Назад", "Вернуться в главное меню"),
            ]
        )
        choice = ask("Выберите действие")
        ...
        try:
            if choice == "1":
                ...
                execute_customer_order(app)
            elif choice == "3":
                show_automation_history(app)
            elif choice == "4":
                post_customer_order(app)
            else:
                continue
        except (ConsultantError, ValueError) as exc:
            ui.error(str(exc))
...
def manual_login(app: Application) -> None:
    if not app.active_connection:
        raise ConsultantError("Сначала выберите базу и подключите проверку данных в разделе «Подключения к базам 1С».")
    if not app.playwright:
        raise ConsultantError("Служба Playwright MCP недоступна.")
    ui.info("Откроется отдельное окно Edge. Войдите в 1С самостоятельно; пароль не попадёт в приложение или AI.")
    with ui.spinner("Открываю защищённое окно входа"):
        app.playwright.start_manual_login(app.active_connection)
    ui.success("Окно входа открыто.")
    ui.pause("После входа в 1С нажмите Enter здесь")
    if app.playwright.finish_manual_login():
        app.browser_login_completed = True
        ui.success("Вход подтверждён. Playwright MCP держит сессию открытой для сценария.")
    else:
        ui.error("1С не подтвердила вход. Проверьте, что в браузере открыта рабочая страница, и повторите вход.")


def execute_customer_order(app: Application) -> None:
    if not app.active_connection:
        raise ConsultantError("Сначала выберите базу в разделе «Подключения к базам 1С».")
    if not app.odata.status()["configured"] or not app.browser_login_completed or not app.playwright or not app.automation:
        raise ConsultantError("Выберите базу, проверьте OData и завершите самостоятельный вход в 1С.")
    request = CustomerOrderRequest()
    run = CustomerOrderAutomation(app.odata, app.playwright, app.automation).run(request)
    ui.panel(
        "Заказ записан",
        [
            f"Клиент: {request.customer}", f"Организация: {request.organization}",
            f"Договор: {request.contract or 'определит 1С'}", f"Товар: {request.product}",
            f"Количество: {request.quantity}", "Цена 100 RUB подставлена автоматически и проверена.",
            f"Номер: {run.values.get('number', '—')}", "Документ записан, но не проведён.", f"Идентификатор запуска: {run.run_id}",
        ],
        "yellow",
    )
    ui.info("Сценарий остановлен перед проведением. Для проведения выберите отдельную команду.")


def show_automation_history(app: Application) -> None:
    runs = app.automation.list() if app.automation else []
    if not runs:
        ui.info("Запусков пока нет.")
        return
    ui.table(["Время", "Сценарий", "Статус", "ID"], [[r.created_at, r.scenario, r.status, r.run_id] for r in runs])


def playwright_mcp_status() -> dict[str, str | bool]:
    config_path = Path(os.environ.get("CODEX_HOME", str(Path.home() / ".codex"))) / "config.toml"
    if not config_path.is_file():
        return {"configured": False, "message": "конфигурация Codex не найдена"}
    text = config_path.read_text(encoding="utf-8", errors="replace")
    has_server = "[mcp_servers.playwright]" in text
    uses_edge = "--browser" in text and "msedge" in text
    if has_server and uses_edge:
        return {"configured": True, "message": "настроен для Microsoft Edge"}
    if has_server:
        return {"configured": False, "message": "найден, но браузер Edge не настроен"}
    return {"configured": False, "message": "не подключён в Codex"}


def show_automation_prompt() -> None:
    ui.clear()
    ui.header("Сценарий для Codex", breadcrumb="Автоматизация 1С › Сценарий")
    ui.panel(
        "Скопируйте этот текст в Codex",
        [
            "Работай через уже открытый веб-интерфейс выбранной базы 1С.",
            "Учётные данные не запрашивай и не вводи: консультант вошёл самостоятельно.",
            "Выполняй действия через Playwright MCP, затем подтверди результат через OData.",
            "Не создавай, не изменяй и не проводи документы без моего отдельного подтверждения.",
            "При неоднозначности остановись и спроси меня.",
        ],
        "green",
    )
    ui.info("Playwright MCP нажимает элементы 1С; OData подтверждает сохранённые данные.")


def post_customer_order(app: Application) -> None:
    if not app.playwright or not app.automation:
        raise ConsultantError("Нет активной сессии или журнала запусков.")
    run_id = ask_required("Идентификатор запуска")
    if not yes_no("Провести этот документ сейчас? Это изменит данные 1С."):
        ui.info("Проведение отменено.")
        return
    run = CustomerOrderAutomation(app.odata, app.playwright, app.automation).post(run_id, True)
    ui.success(f"Заказ {run.values.get('number', '—')} проведён и повторно подтверждён через OData.")
    ...
    while True:
        ...
        ui.menu(
            [
                ...
                ("4", "Расширенные настройки", "Подключение AI, интерфейс и диагностика"),
                ("0", "Назад", "Вернуться в главное меню"),
            ]
        )
        choice = ask("Выберите действие")
        ...
        if choice == "1":
            ...
            advanced_settings_menu(app)


def advanced_settings_menu(app: Application) -> None:
    while True:
        ui.clear()
        ui.header("Расширенные настройки", breadcrumb="Настройки и справка › Расширенные")
        ui.menu(
            [
                ("1", "Подключить AI", "Настроить способ генерации"),
                ("2", "Выбрать интерфейс", "Программа, Codex, OpenCode или Claude"),
                ("3", "Технические данные OData", "Диагностика и чтение данных"),
